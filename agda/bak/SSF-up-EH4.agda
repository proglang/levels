...
Nat         [ l′ ]TL = Nat
(` lskip x) [ l′ ]TL = ` subst (_ ∍_) {!   !} x
(T₁ ⇒ T₂)   [ l′ ]TL = (T₁ [ l′ ]TL) ⇒ (T₂ [ l′ ]TL)
∀α T        [ l′ ]TL = ∀α {! T  !}
∀ℓ T        [ l′ ]TL = ∀ℓ (subst (Type _) {!   !} (T [ Lwk l′ ]TL))
...
lookup-η {κ = κ} η (lskip {l = l} x) = {!  lookup-η η x !} -- lookup-η η x -- cast -- TODO
...
⟦ ∷l Γ   ⟧Γ κ η = {!  ⟦ Γ ⟧Γ (drop-κ κ)  !} -- ⟦ Γ ⟧Γ (drop-κ κ) ? 
