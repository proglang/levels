...
TRen : (ρ : LRen δ₁ δ₂) → TEnv δ₁ → TEnv δ₂ → Set
TRen {δ₁} ρ Δ₁ Δ₂ = ∀ μ (l : Lvl δ₁ μ) → Δ₁ ∍ l → Δ₂ ∍ (Lren ρ l)

Tidᵣ : TRen Lidᵣ Δ Δ
Tidᵣ _ _ = subst (_ ∍_) {!   !} 

Tdropᵣ : ∀ {ρ : LRen δ₁ δ₂} → TRen ρ (l ∷ Δ₁) Δ₂ → TRen ρ Δ₁ Δ₂
Tdropᵣ ρ _ _ x = ρ _ _ (there x)

TTwkᵣ : ∀ {ρ : LRen δ₁ δ₂} → TRen ρ Δ₁ Δ₂ → TRen ρ Δ₁ (l ∷ Δ₂)
TTwkᵣ ρ _ _ x = there (ρ _ _ x)

TLwkᵣ : ∀ {ρ : LRen δ₁ δ₂} → TRen ρ Δ₁ Δ₂ → TRen (Lwkᵣ ρ) Δ₁ (∷l Δ₂)
TLwkᵣ ρ _ _ x = subst (_ ∍_) {!   !} (lskip (ρ _ _ x))

TTliftᵣ : ∀ {ρ : LRen δ₁ δ₂} → TRen ρ Δ₁ Δ₂ → ∀ (l : Lvl δ₁ μ) → TRen ρ (l ∷ Δ₁) (Lren ρ l ∷ Δ₂)
TTliftᵣ ρ _ _ _ (here)      = here
TTliftᵣ ρ _ _ _ (there x)   = there (ρ _ _ x)

TLliftᵣ : ∀ {ρ : LRen δ₁ δ₂} → TRen ρ Δ₁ Δ₂ → TRen (Lliftᵣ ρ) (∷l Δ₁) (∷l Δ₂)
TLliftᵣ {Δ₂ = Δ₂} ρ _ _ (lskip x) = subst ((∷l Δ₂) ∍_) {!   !} (lskip (ρ _ _ x))

Tren : ∀ {ρ : LRen δ₁ δ₂} → TRen ρ Δ₁ Δ₂ → Type Δ₁ l → Type Δ₂ (Lren ρ l)
Tren ρ Nat       = Nat
Tren ρ (` x)     = ` ρ _ _ x
Tren ρ (T₁ ⇒ T₂) = Tren ρ T₁ ⇒ Tren ρ T₂
Tren ρ (∀α T)    = ∀α (Tren (TTliftᵣ ρ _) T)
Tren ρ (∀ℓ T)    = ∀ℓ (subst (Type _) {!   !} (Tren (TLliftᵣ ρ) T))
-- 
-- -- 
-- -- postulate 
...
TTwk T = subst (Type _) {!   !} (Tren (TTwkᵣ Tidᵣ) T)
-- 
-- postulate
TLwk : Type Δ l′ → Type (∷l Δ) (Lwk l′)
TLwk = Tren (TLwkᵣ Tidᵣ)

TSub : (σ : LSub δ₁ δ₂) → TEnv δ₁ → TEnv δ₂ → Set
TSub {δ₁} σ Δ₁ Δ₂ = ∀ μ (l : Lvl δ₁ μ) → Δ₁ ∍ l → Type Δ₂ (Lsub σ l)

Tidₛ : TSub Lidₛ Δ Δ
Tidₛ _ _ x = subst (Type _) {!   !} (` x)

Tdropₛ : ∀ {σ : LSub δ₁ δ₂} → TSub σ (l ∷ Δ₁) Δ₂ → TSub σ Δ₁ Δ₂
Tdropₛ σ _ _ x = σ _ _ (there x)

TTwkₛ :  ∀ {σ : LSub δ₁ δ₂} → TSub σ Δ₁ Δ₂ → TSub σ Δ₁ (l ∷ Δ₂)
TTwkₛ σ _ _ x = TTwk (σ _ _ x)

TTliftₛ : ∀ {σ : LSub δ₁ δ₂} → TSub σ Δ₁ Δ₂ → ∀ (l : Lvl δ₁ μ) → TSub σ (l ∷ Δ₁) (Lsub σ l ∷ Δ₂)  
TTliftₛ σ _ _ _ here = ` here
TTliftₛ σ _ _ _ (there x) = TTwk (σ _ _ x)

TLliftₛ : ∀ {σ : LSub δ₁ δ₂} → TSub σ Δ₁ Δ₂ → TSub (Lliftₛ σ) (∷l Δ₁) (∷l Δ₂)  
TLliftₛ σ _ _ (lskip x) = subst (Type _) {!   !} (TLwk (σ _ _ x))

Tsub : ∀ {σ : LSub δ₁ δ₂} → TSub σ Δ₁ Δ₂ → Type Δ₁ l → Type Δ₂ (Lsub σ l)
Tsub σ Nat       = Nat
Tsub σ (` x)     = σ _ _ x
Tsub σ (T₁ ⇒ T₂) = Tsub σ T₁ ⇒ Tsub σ T₂
Tsub σ (∀α T)    = ∀α (Tsub (TTliftₛ σ _) T)
Tsub σ (∀ℓ T)    = ∀ℓ (subst (Type _) {!   !} (Tsub (TLliftₛ σ) T))
-- 
...
--     
TTextₛ : ∀ {σ : LSub δ₁ δ₂} → TSub σ Δ₁ Δ₂ → Type Δ₂ (Lsub σ l) → TSub σ (l ∷ Δ₁) Δ₂
TTextₛ σ T′ _ _ here      = T′
TTextₛ σ T′ _ _ (there x) = σ _ _ x

TLextₛ : ∀ {σ : LSub δ₁ δ₂} → TSub σ Δ₁ Δ₂ → TSub (Lextₛ σ l) (∷l Δ₁) Δ₂
TLextₛ σ _ _ (lskip x) = subst (Type _) {!   !} (σ _ _ x)

_[_]TT : Type (l ∷ Δ) l′ → Type Δ l → Type Δ l′
T [ T′ ]TT = subst (Type _) {!   !} (Tsub (TTextₛ Tidₛ (subst (Type _) {!   !} T′)) T)

_[_]TL : ∀ {Δ : TEnv δ} {l : Lvl (tt ∷ δ) μ} →
    Type (∷l Δ) l → (l′ : Lvl δ <ω) → Type Δ (l [ l′ ]LL)
T [ l′ ]TL = Tsub (TLextₛ Tidₛ) T 

_T≫ᵣᵣ_ : ∀ {ρ₁ : LRen δ₁ δ₂} {ρ₂ : LRen δ₂ δ₃} → TRen ρ₁ Δ₁ Δ₂ → TRen ρ₂ Δ₂ Δ₃ → TRen (ρ₁ L≫ᵣᵣ ρ₂) Δ₁ Δ₃
(ρ₁ T≫ᵣᵣ ρ₂) _ _ x = subst (_ ∍_) {!   !} (ρ₂ _ _ (ρ₁ _ _ x))

_T≫ᵣₛ_ : ∀ {ρ₁ : LRen δ₁ δ₂} {σ₂ : LSub δ₂ δ₃} → TRen ρ₁ Δ₁ Δ₂ → TSub σ₂ Δ₂ Δ₃ → TSub (ρ₁ L≫ᵣₛ σ₂) Δ₁ Δ₃
(ρ T≫ᵣₛ σ) _ _ x = subst (Type _) {!   !} (σ _ _ (ρ _ _ x))

_T≫ₛᵣ_ : ∀ {σ₁ : LSub δ₁ δ₂} {ρ₂ : LRen δ₂ δ₃} → TSub σ₁ Δ₁ Δ₂ → TRen ρ₂ Δ₂ Δ₃ → TSub (σ₁ L≫ₛᵣ ρ₂) Δ₁ Δ₃
(σ T≫ₛᵣ ρ) _ _ x = subst (Type _) {!   !} (Tren ρ (σ _ _ x))

_T≫ₛₛ_ : ∀ {σ₁ : LSub δ₁ δ₂} {σ₂ : LSub δ₂ δ₃} → TSub σ₁ Δ₁ Δ₂ → TSub σ₂ Δ₂ Δ₃ → TSub (σ₁ L≫ₛₛ σ₂) Δ₁ Δ₃
(σ₁ T≫ₛₛ σ₂) _ _ x =  subst (Type _) {!   !} (Tsub σ₂ (σ₁ _ _ x))
