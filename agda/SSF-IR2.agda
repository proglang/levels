...
encode {Δ = Δ} (∀α {l = l} {l′ = l′} T) κ η = Π' (U' (<≤-trans ℕ*ℕ.<suc (⊔₁ _ _))) λ A → 
    ...
    Lift≤ (⊔₂ _ _) T
encode (∀ℓ {l = l} T) κ η = Π' ℕ' λ l′ → 
    let T = coe ((cong U (⟦Lwk⟧L l κ l′))) (encode T (l′ ∷κ κ) η) in 
    Lift≤ (⊔₂ ω _) T
...
⟦_⟧E {T = (_⇒_ {l₁ = l₁} T₁ T₂)} {Γ} (λx e) κ η γ = λ x → 
  let e = ⟦ e ⟧E κ η (_∷γ_ {T = T₁} {Γ = Γ} (coe (ElLift≤ (⊔₁ _ _) (encode T₁ κ η)) x) γ) in 
  coe (sym (ElLift≤ (⊔₂ (⟦ l₁ ⟧L κ) _) (encode T₂ κ η))) e
⟦_⟧E {Δ = Δ} {T = ∀α T} (Λ l ⇒ e) κ η γ = λ A → 
  let eq = (Uⁱʳ & ext (λ j → ext (λ p → cong (λ acc → (U< {⟦ l ⟧L κ} ⦃ acc ⦄ j p)) (Acc-prop _ wf)))) in
  let η′ =  _∷η_ {l = l} {Δ = Δ} {κ = κ} (coe eq A) η in
  let e = ⟦ e ⟧E κ η′ γ in 
  coe (sym (ElLift≤ (⊔₂ (⟦ `suc l ⟧L κ) _) (encode T κ η′))) e
⟦_⟧E {T = ∀ℓ {l = l′} T} (Λℓ e) κ η γ = λ l → 
  let e = ⟦ e ⟧E (l ∷κ κ) η γ in 
  let eq₁ = ElLift≤ (⊔₂ ω (⟦ l′ ⟧L κ)) (coe (cong U (⟦Lwk⟧L l′ κ l)) (encode T (l ∷κ κ) η)) in
  let eq₂ = {!   !} in
  coe (sym (trans eq₁ eq₂)) e
  -- where crucial : ∀ l → 
  --         Elⁱʳ {⟦ l′ ⟧L κ} (coe ((λ i → Uⁱʳ (λ j p → Uⁱʳ U<)) & ⟦Lwk⟧L l′ κ l) (encode T (l , κ) η)) ≡ 
  --         Elⁱʳ {⟦ Lwk l′ ⟧L (_ ∷κ κ)} (encode T (l ∷κ κ) η)
  --       crucial l  = {!   !}
⟦ e₁ · e₂ ⟧E κ η γ = {!   !}
⟦ e ∙ T′  ⟧E κ η γ = {!   !}
⟦ e ∙ℓ l′ ⟧E κ η γ = {!   !}