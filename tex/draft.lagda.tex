...
By an abuse of notation we may write $\texttt{f} : ∀(ℓ : \texttt{Level}_{ℓ′}) → Set ℓ$ and $\texttt{f} \ ℓ \ \{ℓ<ℓ′}$ instead of $\texttt{f} : ∀(ℓ : \texttt{Level}_{ℓ′}) → \texttt{Set } (\texttt{proj}_ℓ \ ℓ)$ and $\texttt{f} (ℓ ,_ℓ ℓ<ℓ′)$ which is closer to what we believe should be implemented.
...
We could implement them in an manually checked unsafe module and mark them for the compiler.
